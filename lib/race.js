const { generateRace } = require('./generateRace');
const { payout } = require('./betting.js');
...
function raceRun(raceId, db, io) {
    ...
    const interval = setInterval(() => {
        ...
        if (order.length === horses.length) {
            ...
            // possibly make payout async ? if not lol
...
function derby(db, io) {
    let raceId = generateRace(db);
    const row = db.prepare('SELECT start_time FROM races WHERE id = ?').get(raceId);
    const startTime = new Date(row.start_time).getTime();
    io.emit('nextRace', { startTime: startTime });
