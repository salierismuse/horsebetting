...
function payout(raceId, db) {
    const winner = db.prepare('SELECT winner_horse_id FROM races WHERE race_id = ?').get(raceId).winner_horse_id;
    let totalAmount = db.prepare('SELECT SUM(amount) FROM bets WHERE race_id = ?').get(raceId).total || 0;
    ...
    for (let i of bets) {
        ...
        db.prepare('UPDATE users SET balance = balance + ? WHERE id = ?').run(payout, bets[i].user_id);
    }
    })();
};
